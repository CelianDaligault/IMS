...
spark = SparkSession.builder.appName("WeatherPrediction").getOrCreate() #We create the spark session
df = spark.read.csv("weather_data.csv", header=True, inferSchema=True) #We read the csv file
df = df.withColumn("timestamp", to_timestamp(col("utc_timestamp"))).drop("utc_timestamp")
df = df.dropna() 

# We group the data by timestamp and calculate the mean and standard deviation of temperature and radiation
...
# assembler = VectorAssembler(inputCols=["mean_temp", "mean_radiation_direct", "mean_radiation_diffuse"], outputCol="features") #We create the vector assembler
...
plt.figure(figsize=(10, 5))
plt.scatter(df_pandas["timestamp"], df_pandas["mean_temp"], label="Mean Temperature FR", color='b', s=5)
plt.xlabel("Date")
plt.ylabel("Mean Temperature (°C)")
plt.legend()
plt.grid()
